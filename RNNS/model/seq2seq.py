...
class Seq2seq(nn.Module):
	...
	def __init__(self, embedding=None, wordDict=None, hidden_size=300, style_size=100, input_dropout_p=0, max_len=100, dropout_p=0, n_layers=1, bidirectional=False, rnn_cell='gru', use_lang_model=False, decode_function=F.log_softmax):
	...
	def forward(self, inputs, target_variable=None,
		...
		if self.training and self.use_lang_model:
...
class Criterion(nn.Module):
	...
	def __init__(self, config):
		...
		with open(config['model']['wordDict'],"rb") as fp:
			self.wordDict = pickle.load(fp)

		if config['model']['use_lang_model']==1 or config['evaluator']['lm_eval']==1:
	...
	def forward(self, outputs, inputs):
		...
		if self.config['model']['use_lang_model']==1:
			# TODO: use cat & split
