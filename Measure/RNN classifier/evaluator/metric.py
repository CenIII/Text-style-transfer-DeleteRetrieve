...
class Metrics:
    def __init__(self, model_path, bleu_reference_path, net):
    ...
    def reloadClassifierModel(self, model, model_path):
        ...
        print("=> Reloading checkpoint '{}': model".format(model_path + '/bestmodel_new.pth.tar'))
        ...
        # 3. has little to do with the style 
                    if output[0][0] > output[0][1]:
                        if part_output[0][0]<min(0.6,output[0][0]) and part_output[0][0]>max(0.4, output[0][1]):
                            marker_words.append(sentence[i])
                    else:
                        if part_output[0][0]<min(0.6,output[0][1]) and part_output[0][0]>max(0.4, output[0][0]):
                            marker_words.append(sentence[i])
                print(sentence)
                print('marker words are', marker_words)
                print()
        extract(net, 0)
        extract(net, 1)
    ...
    def word2index(self, sentence):
        with open('../../AuxData/wordDict',"rb") as fp:
